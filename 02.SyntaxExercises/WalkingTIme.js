function walkingTime(steps, lengthOfFoot, speed){

    let metersToUniversity =steps * lengthOfFoot;
    let speedInSeconds= speed / 3.6;
    let timeInSeconds = metersToUniversity / speedInSeconds;

    let secondsToWalk = timeInSeconds % 60;
    let minutesToWalk = Math.floor(timeInSeconds / 60);

    let minutesBreak = Math.floor(metersToUniversity / 500);
